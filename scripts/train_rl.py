...
def train(gpu, args):
    ...
    proc_per_gpu = args.procs // args.gpus # no. of env proc to launch per gpu this node
    ...
    # if acmodel is None:
    if args.pretrained_model:
        acmodel = utils.load_model(args.pretrained_model, raise_not_found=True)  # need to copy pretrained model to
    ...
    while status['num_frames'] < args.frames:
        ...
        for k, v in logs.items():
            all_logs[k] = [None for _ in range(args.ws)]
