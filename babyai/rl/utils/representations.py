...
class StateAction(object):
    ...
    def __init__(self, spm, vocab_kge, vocab, vocab_rev):
    ...
    def update_state(self, obs, prev_action, env, cache=None):
        ...
        if env.carrying:
            ...
            agent_rules.append(('You', 'have', obj_desc['name']))
        ...
        if self.room != prev_room:
            ...
            door_desc = self.get_obj_desc(self.entry_door, env)
            rules.append((door_desc['name'], 'connects', self.room))
    ...
    def get_action_rep_drqa(self, action, env):
        return self.vocab_act_rev[env.Actions(action).name]

    def step(self, obs, prev_action, env, cache=None):
        rules, agent_rules = self.update_state(obs, prev_action, env, cache)
